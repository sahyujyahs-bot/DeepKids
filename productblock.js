...
(function () {
  ...
  function wireEta() {
    ...
    document.querySelectorAll('.buy-actions').forEach(function (row) {
      ...
      more.innerHTML = '<a href="/">Check out our other products &rarr;</a>';
      eta.appendChild(more);
    });
  }

  /* Once the block has scrolled past, the bar carries a way back to
     it — otherwise the only Add To Cart on the page is somewhere above
     and there is nothing to press. EscapeGravity builds its own, on
     its own timing, so this stands aside there. */
  function wireNavCta() {
    if (document.querySelector('.eg-nav-cta, .dkb-nav-cta')) return;
    if (!window.hydrateDkBtn) return;
    /* Each in turn, not one comma-separated query: that returns the
       first match in document order, which is the outer bar rather than
       the group of controls inside it. */
    var bar = document.querySelector('.eg-nav-right')
           || document.querySelector('.eg-nav')
           || document.querySelector('.mast');
    var block = document.querySelector('.buy-wrap');
    if (!bar || !block) return;

    var btn = document.createElement('button');
    btn.type = 'button';
    btn.className = 'dkb-nav-cta';
    btn.setAttribute('data-dk-size', 'sm');
    btn.setAttribute('data-dk-btn', 'Order Now');
    btn.setAttribute('aria-label', 'Back to the order form');
    bar.insertBefore(btn, bar.firstChild);
    window.hydrateDkBtn(btn);

    btn.addEventListener('click', function () {
      block.scrollIntoView({ behavior: 'smooth', block: 'start' });
    });

    /* Shown once the block's own button is off the top of the screen,
       hidden again the moment it is back — two Order Now buttons on
       one screen is one too many. */
    var t = false;
    function paint() {
      t = false;
      btn.classList.toggle('show', block.getBoundingClientRect().bottom < 90);
    }
    window.addEventListener('scroll', function () {
      if (!t) { t = true; requestAnimationFrame(paint); }
    }, { passive: true });
    window.addEventListener('resize', paint);
    paint();
  }
  window.addEventListener('load', wireNavCta);
  document.addEventListener('DOMContentLoaded', wireNavCta);
