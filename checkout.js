...
(function () {
  ...
  function dkCoOpen() {
    ...
    /* Carry over whatever pincode they already checked, and keep the
       date under the field answering as they retype it. */
    var pinEl = document.getElementById('co-pin');
    if (pinEl && !pinEl.value && DK.pin) pinEl.value = DK.pin();
    coEta(false);
  ...
  /* The estimate under the pincode field. Same numbers as the cart
     drawer and the product blocks — they all call DK.eta. */
  function coEta(settled) {
    var out = document.getElementById('co-eta');
    var pinEl = document.getElementById('co-pin');
    if (!out || !pinEl) return;
    var v = pinEl.value.replace(/\D/g, '').slice(0, 6);
    if (pinEl.value !== v) pinEl.value = v;
    pinEl.classList.remove('bad');
    out.className = 'co-eta';
    if (v.length < 6) {
      /* Half a pincode is only wrong once they stop typing it. */
      if (settled && v) { pinEl.classList.add('bad'); out.textContent = 'A pincode is six digits.'; }
      else out.textContent = '';
      return;
    }
    var e = DK.eta && DK.eta(v);
    if (!e) {
  ...
  document.getElementById('co-pin').addEventListener('input', function () { coEta(false); });
  document.getElementById('co-pin').addEventListener('blur',  function () { coEta(true); });
